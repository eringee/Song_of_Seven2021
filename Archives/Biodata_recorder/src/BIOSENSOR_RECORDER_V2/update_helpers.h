    ...
    switch(i)
    {
      case 0:
        ...
        {   
            ...
            setLedBrightness(i , sensorData[i]*0.7);
        }
        break;
      case 1:
        if(connectedSensors[i])
      ...
      case 2:
        if(connectedSensors[i])
        {
          ...
          //sensorData[i] = (0.1*resp.getNormalized()) + ((1-0.1)*resp.getNormalized());    //added lowpass filter thorugh exponential moving average
          bufferA.push(sensorData[i]);
          bufferA.push(resp.bpmChange());
          bufferA.push(resp.amplitudeChange());
          //bufferA.push(0.5);
          //bufferA.push(0.5);
          if (sensorData[i]<0.2) sensorData[i]=0;
          setLedBrightness(i , sensorData[i]*0.95);
